...
- **프로덕션 코드에는 주석을 가능한 한 쓰지 않는다.** 의도는 코드 자체(좋은 이름·작은 함수·명확한 타입·구조)로 드러낸다.
- **주석이 필요하다고 느껴지면, 그건 "코드만으로 의도가 안 읽힌다"는 신호다 → 리팩터링 대상으로 본다.** 주석을 먼저 다는 게 아니라, 더 나은 이름·함수 추출·구조 변경으로 **주석 없이도 읽히게** 코드를 고친다. (이미 달린 주석을 발견하면 같은 신호로 보고 `code-refactoring`으로 다룬다.)
- **예외(주석 허용)**: ① 테스트 코드, ② DTO 등 **스펙·외부 계약을 문서화하는 코드**(huma DTO 필드 설명/OpenAPI 문서 주석, 공개 API의 godoc, `doc.go` 패키지 문서). 이들은 주석/태그가 외부 계약·문서를 표현하는 정당한 수단이다.
- 즉 금지 대상은 **구현 본문을 한국어로 반복하는 설명형 주석**(`// roomID로 세션 조회` 같은 것), TODO 누적, 죽은 코드 설명이다. godoc·문서화 관용 자체를 막는 게 아니다 — 문서화는 `golang-documentation` 스킬을 따른다.
